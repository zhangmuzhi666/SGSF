...
class FABAttack:
    ...
    def projection_linf(self, points_to_project, w_hyperplane, b_hyperplane):
        ...
        c5 = (w < 0).float()
        a = torch.ones(t.shape, device=device)  # 使用device而不是self.device
        d = (a * c5 - t) * (w != 0).float()
        a -= a * (1 - c5)

        p = torch.ones(t.shape, device=device) * c5 - t * (2 * c5 - 1)  # 使用device
        ...
        u = torch.arange(0, w.shape[0], device=device)  # 确保在正确设备上
        ...
        # 确保lb和ub在正确的设备上
        lb = torch.zeros(c2.shape[0], device=device)
        ub = torch.ones(c2.shape[0], device=device) * (w.shape[1] - 1)
        nitermax = torch.ceil(torch.log2(torch.tensor(w.shape[1]).float()))
        counter2 = torch.zeros(lb.shape, device=device).long()
        ...
        while counter < nitermax:
            ...
            # 确保所有张量在同一设备上
            if lb.device != ind3.device:
                ind3 = ind3.to(lb.device)
                counter4 = counter4.to(lb.device)
            lb[ind3] = counter4[ind3]

            if lb.device != ind32.device:
                ind32 = ind32.to(lb.device)
                counter4 = counter4.to(lb.device)
        ...
        if c_l.nelement != 0:
            lmbd_opt = (
                torch.max(
                    ...
                    torch.zeros(sb[c_l, -1].shape, device=device),  # 使用device
                )
            ).unsqueeze(-1)
        ...
        lmbd_opt = (
            torch.max(
                ...
                torch.zeros(sb[c2, lb].shape, device=device),  # 使用device
            )
        ).unsqueeze(-1)
    ...
    def projection_l2(self, points_to_project, w_hyperplane, b_hyperplane):
        ...
        u = torch.arange(0, w.shape[0], device=device).unsqueeze(1)  # 确保在正确设备上

        r = torch.max(t / w, (t - 1) / w)
        u2 = torch.ones(r.shape, device=device)  # 使用device而不是self.device
        ...
        rs2 = torch.cat(
            (rs[:, 1:], torch.zeros(rs.shape[0], 1, device=device)), 1
        )  # 使用device
        rs[rs == 1e12] = 0
        rs2[rs2 == 1e12] = 0
        ...
        lb = lb.long()
        alpha = torch.zeros([1], device=device)  # 确保alpha在正确设备上
    ...
    def projection_l1(self, points_to_project, w_hyperplane, b_hyperplane):
        ...
        r = torch.max(1 / w, -1 / w)
        r = torch.min(
            r, 1e12 * torch.ones(r.shape).to(device)
        )  # 使用device而不是self.device
        rs, indr = torch.sort(r, dim=1)
        ...
        # 确保lb和ub在正确的设备上
        lb = torch.zeros(c2.shape[0], device=device)
        ub = torch.ones(c2.shape[0], device=device) * (s.shape[1])
        ...
        if c2.nelement() != 0:
            ...
            d[c2] = d[c2] * u3.float().to(device)  # 使用device而不是self.device
