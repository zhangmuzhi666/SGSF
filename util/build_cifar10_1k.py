"""
build_cifar10_1k.py
构建或加载 CIFAR-10 的 1k 均衡子集（每类100样本，保持原始顺序）。
保存路径：data/cache/cifar10/cifar10_1000_balanced.pth
"""
...
CACHE_DIR = DATA_ROOT / "cache" / "cifar10"
CACHE_DIR.mkdir(parents=True, exist_ok=True)

SUBSET_FILE = CACHE_DIR / "cifar10_1k_balanced.pth"


def build_cifar10_1k_subset(dataset=None, per_class_k=100, overwrite=False):
    """
    构建 CIFAR-10 均衡子集（每类 per_class_k 个，默认100，共1000个）。
    ...
    torch.save(
        {
            "indices": indices,
            "labels": labels,
            "meta": {"dataset": "cifar10", "per_class_k": per_class_k},
        },
        SUBSET_FILE,
    )
...
def subset_loader_from_pth(
    ...
    根据缓存文件创建 DataLoader。
    """
    # 显式设置为 False，表示信任此文件且可能包含非权重数据
    info = torch.load(SUBSET_FILE, weights_only=False)
...
if __name__ == "__main__":
    ...
    print("==== 构建 CIFAR-10 1k 均衡子集 ====")
    download_cifar10()
    ds = get_dataset(train=False)
    subset_pth = build_cifar10_1k_subset(dataset=ds)
